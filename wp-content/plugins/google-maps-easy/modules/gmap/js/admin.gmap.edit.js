...
jQuery(document).ready(function(){
	...
	jQuery('#gmpSaveMarkerBtn').click(function(){
		var markerId = jQuery('#gmpMarkerForm input[name="marker_opts[id]"]').val();
		jQuery('#gmpMarkerForm').data('edit', markerId).submit();
		return false;
	});
		...
		if(confirm('Remove '+ markerTitle+ ' marker?')) {
			...
			var removeFinalClb = function() {
				if(currentMarkerIdInForm) {
					...
					jQuery('#gmpMarkersListGrid').jqGrid('delRowData', currentMarkerIdInForm);
				}
				if(g_gmpCurrentEditMarker) {
					g_gmpCurrentEditMarker.removeFromMap();
				}
				gmpResetMarkerForm();
				//gmpRefreshMapMarkersList( true );		// For old markers list version
			};
			if(currentMarkerIdInForm) {
				jQuery.sendFormGmp({
					btn: this
	...
	jQuery('#gmpMarkerForm').submit(function(){
		...
		var editMarker = parseInt(jQuery(this).data('edit'))
				...
				if(!res.error) {
					if(!onlySave) {
						...
						if(!res.data.update) {
							...
							marker = g_gmpMap.getMarkerById( res.data.marker.id );
						}
						if(marker) {
							marker.setParams( res.data.marker );
						}
					}
					...
					if(!onlySave) {
						var coords = gmpGetMarkerCoordsString(res.data.marker.coord_x, res.data.marker.coord_y)
						,	markerData = {
								id: res.data.marker.id
							,	icon_img: '<div class="egm-marker-icon"><img src="' + res.data.marker.icon_data.path + '" /></div>'
							,	title: res.data.marker.title
							,	coords: coords
							,	actions: res.data.marker.actionButtons.replace(/\r|\n/g, '')
						};
						if (editMarker) {
							jQuery('#gmpMarkersListGrid').jqGrid('setRowData', res.data.marker.id, markerData);
						} else {
							jQuery('#gmpMarkersListGrid').jqGrid('addRowData', res.data.marker.id, markerData, 'last');
						}
					}
					//gmpRefreshMapMarkersList( true, (onlySave ? true : false) );	// For old markers list version
	...
	//gmpRefreshMapMarkersList(false, true);	// For old markers list version
	...
	,	colModel: [
			{ name: 'id', index: 'id', key: true, hidden: true, sortable: false, width: '10', align: 'center' }
...
function gmpGetMarkerCoordsString(coordX, coordY) {
	return '<div class="egm-marker-latlng">' + parseFloat(coordX).toFixed(2)+ '"N ' + parseFloat(coordY).toFixed(2) + '"E</div>';
}
...
function gmpRemoveMarkerFromMapTblClick(markerId, params) {
	params = params || {};
	var markerTitle = params.row ? params.row.find('td[aria-describedby="gmpMarkersListGrid_title"]').text() : '' // find .egm-marker-title for old markers list version
	...
	,	onSuccess: function(res) {
			if(!res.error){
				...
				jQuery('#gmpMarkersListGrid').jqGrid('delRowData', markerId);
...
function _gmpMarkerDragEndClb() {
	...
	if(draggedId) {	// Just save it in database
		...
		,	onSuccess: function(res) {
				if(!res.error) {
					var marker = g_gmpMap.getMarkerById(draggedId);
					marker.setParam('coord_x', self.lat());
					marker.setParam('coord_y', self.lng());
					jQuery('#gmpMarkersListGrid').jqGrid('setCell', draggedId, 'coords', gmpGetMarkerCoordsString(self.lat(), self.lng()));
					//gmpRefreshMapMarkersList( true );		// For old markers list version
				}
			}
		});
...
function _gmpResizeRightSidebar() {
	...
	if(rightBarHt > wndHt && wndWd <= 800) {
		...
		,	markersListHeaderHeight = jQuery('#markerList').find('table thead').height()
