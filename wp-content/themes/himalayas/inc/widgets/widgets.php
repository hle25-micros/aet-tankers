...
function himalayas_widgets_init() {
   ...
   register_widget( "himalayas_our_business_widget" );
 ...
 */
class himalayas_our_business_widget extends WP_Widget {
   function __construct() {
      $widget_ops = array( 'classname' => 'widget_our_business_block', 'description' => __( 'Show Our Business.', 'himalayas' ) );
      $control_ops = array( 'width' => 200, 'height' =>250 );
      parent::__construct( false, $name = __( 'TG: Our Business Widget', 'himalayas' ), $widget_ops, $control_ops);
   }
   ...
   function form( $instance ) {
      ...
      <p><?php _e( 'Note: Enter the Our Business Section ID and use same for Menu item. Only used for One Page Menu.', 'himalayas' ); ?></p>
      <p>
         <label for="<?php echo $this->get_field_id( 'team_menu_id' ); ?>"><?php _e( 'Our Business Section ID:', 'himalayas' ); ?></label>
      ...
      <p><?php _e( 'Note: Create the pages and select Our Business Template to display Our business pages.', 'himalayas' ); ?></p>
   <?php }
   ...
   function widget( $args, $instance ) {
      ...
      foreach ( $pages as $page ) {
         $page_id = $page->ID;
         $template_name = get_post_meta( $page_id, '_wp_page_template', true );
         if( $template_name == 'page-templates/template-business.php' ) {
            array_push( $page_array, $page_id );
         }
      }
